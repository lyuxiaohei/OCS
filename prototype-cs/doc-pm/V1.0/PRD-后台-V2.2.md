...
| 新建表 | OCS 新建表存储 | 知识库文章+分类（建议 `customer_chat_knowledge_base`/`kb_categories`）、留言（建议 `customer_chat_leave_messages`）|
...
| 账号 | 姓名 | 角色 | 用途 |
|------|------|------|------|
| 管理端测试号 | 王小明 / 张小明 / 刘洋 等 | A / S | 后台 11 模块全流程（原型内 Mock 账号）|
...
| 渠道（V2.2）| channel | 两层：品牌商城(父) / 应用(子，H5/PC/…) | 会话创建时落 `entry_channel` | 会话项 + 历史会话 渠道标签 |
...
>
> **渠道标识 2 层模型（V2.2）**：渠道分两层——**父层 = 品牌商城**（平安/浙商/…，按租户 `customer_id` 归属，单租户部署下恒定）；**子层 = 应用**（H5 / PC / 小程序 / …，按用户进线入口判定）。会话/会话项展示的「渠道标签」取**应用层**，**一期仅 H5 一个应用 → 一律显示「H5」**；父层为预留维度，多商城共用一套客服系统时才在 UI 区分（会议：现在不强制显示父层，后续多渠道再展示）。
...
| UC-历史会话-06 | S | 差评追踪 | 差评行（≤2 星）「差评追踪」 | 弹窗 `modal-bad-rating.html`（S 发起、A 补救，ADR-0002）；**一期=记录跟进+主管手动补偿、不挂绩效（Q5）**|
| UC-历史会话-07 | A | 导出/刷新 | 点导出/刷新 | toast 反馈 |
| UC-历史会话-08 | A | 按用户归集视图（V2.2）| 切「按用户」视图 | 同一用户多次会话折叠为一条（头像+昵称+会话数角标+最近时间），展开查看该用户全部会话的完整对话流；与默认「按会话」视图互斥 |
| UC-历史会话-09 | A | 再次联系（V2.2）| 行「再次联系」 | 跳转工作台对该用户发起新会话/主动建联（**主动消息推送本身属 2 期**；一期为客服手动发起入口，用户在线则建联、离线转留言） |
...
| AC-历史会话-05 | UC-历史会话-08 | 列表 | 切「按用户」视图 | 按 `user_id` 聚合：同用户多会话折叠为分组行（会话数角标 + 最近会话时间），展开显示该用户全部会话 + 完整对话流；切回「按会话」恢复逐条列表 |
...
> **V2.2 视图模式**：默认「按会话」逐条列表；新增「按用户」归集视图（UC-历史会话-08）——同 `user_id` 多次会话折叠展示、支持查看完整对话流（会议要求「同一用户多次沟通集中显示」）。两种视图互斥切换。
...
| 渠道列（V2.2）| 应用层标签（一期仅「H5」） | `col-channel` |
| 客服列 | 单名 / 转接双名「张小明 → 李静」 / **连环全链「A → B → C」**（限深 3 跳，每跳独立转接单 + TTL，ADR-0003）|
...
| 编号 | 角色 | 场景 | 动作 | 预期结果 |
|------|------|------|------|----------|
| UC-留言-01 | S | 查看留言统计 | 进入页面 | 4 项：待处理/处理中/已回复/无效 |
| UC-留言-02 | S | 筛选留言 | 设搜索/分类/处理人/时间 + 状态 Tab → 查询 | 列表过滤 |
| UC-留言-03 | S | 分配处理人 | 待处理行「分配」→ 抽屉选处理人 → 确认 | 状态待处理→处理中，处理人列显示，「分配」按钮消失，回复按钮解锁 |
| UC-留言-04 | S | 回复留言 | 处理中行「继续回复」→ 抽屉填回复 → 确认 | 状态→已回复，操作变「查看回复/详情」 |
| UC-留言-05 | S | 标记无效 | 待处理行「标记无效」→ 弹窗 → 确认 | 状态→无效，行变灰，操作变「详情/撤销无效」 |
| UC-留言-06 | S | 撤销无效 | 无效行「撤销无效」 | 状态→待处理，恢复样式 |
...
| pending | process(处理中) | 主管分配 | S | **强制：必须先分配** |
| process | replied(已回复) | 处理人回复 | S | 已分配 |
| pending | invalid(无效) | 标记无效 | S | 广告/骚扰/重复 |
| invalid | pending | 撤销无效 | S | 可逆 |
...
| 无效留言 | 不计入考核统计 |
| 通知通道（ADR-0004）| 留言**回复不主动推送**用户（短信/微信/站内信均不推），改为**回灌用户下次进线会话**在会话内展示；代价：用户不再来则看不到回复。系统设置「留言通知方式」语义已同步调整。 |
...
| 离线语 | 允许留言 | switch | 开 | — |
| 离线语 | 留言通知方式 | select 3 | 站内消息 | **语义调整（ADR-0004）**：留言**回复不主动推送给用户**、改为回灌下次进线会话；此下拉仅控制「新留言到达时如何通知客服」（站内消息/短信/企业微信），与用户侧推送无关 |
...
| 回复 | 处理人回复 | reply_content/status=replied/replied_at | **不主动推送；回复回灌用户下次进线会话时在会话内展示**（ADR-0004）|
...
| 6 | 留言回复回灌（ADR-0004）| 不主动推送（短信/微信/站内信均不推）| — | 用户下次进线时在会话内展示回复；系统设置「留言通知方式」下拉语义同步调整 |
