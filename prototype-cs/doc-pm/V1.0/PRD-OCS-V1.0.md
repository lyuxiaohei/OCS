...
> 范围口径：**1 期 MVP**（16 个原型页面 = 管理端 11 页 + 用户端 5 页，集成于供应链系统统一鉴权）；2/3 期内容以 <span style="color:#999">灰字</span> 标注，不纳入本期上线范围（详见 1.2）
> 角色口径：本 PRD 将业务逻辑清单的 5 角色（User/Agent/Admin/Backend/DB）合并为 **3 角色**（U/A/SYS），SYS 含管理员人工操作 + 后端系统自动化，状态流转「触发角色」列文字区分手动/自动
> 数据源：《业务逻辑清单_V1.0.md》《PROTOTYPE-FLOWS.md》（覆盖 20 页，本 PRD 取其中 1 期 16 页）
...
>
> **16 页拆分明细**：管理端 11 页（仪表盘、工作台、待办、历史会话、客户信息、转接、评价管理、留言管理、知识库、话术库、系统设置）+ 用户端 5 页（入口、聊天、FAQ、离线留言、服务评价）。排除 AI 三页（ai-bot-config/ai-quality/ai-report，3 期）。
>
...
> 注：客服后台集成于供应链系统，登录/鉴权/退出由供应链系统统一负责，客服模块不做自有登录，本 PRD 不再单独设「登录认证」模块。
...
| 编号 | 对应用例 | Given | When | Then |
|------|:------:|-------|------|------|
| AC-W-01 | UC-W-01 | 进入仪表盘 | 页面加载 | 4 张指标卡展示（今日会话/在线客服/平均响应/满意度）；后端 `/api/backend/dashboard` 超时→降级占位 |
...
|---|------|----------|----------|----------|--------|
| 1 | 仪表盘数据 | GET `/api/backend/dashboard` | 时间范围 | 指标卡数据 | 超时降级 |
| 2 | 客服接入会话 | POST `/api/backend/session/accept` | session_id | accept 状态 | 已被接入（并发） |
| 3 | 发起转接 | POST `/api/backend/transfer/create` | session_id/to_admin_id/reason | 转接单 | 目标满载 |
| 4 | 接受转接 | POST `/api/backend/transfer/accept` | transfer_id | 接受成功 | 容量满载/并发 |
| 5 | 留言分配 | POST `/api/backend/leave/assign` | leave_id/assignee_id/priority | 处理中 | 状态非待处理 |
| 6 | 留言回复 | POST `/api/backend/leave/reply` | leave_id/content/method/result | 已回复 | 未分配（强制先分配） |
| 7 | 用户提交评价 | POST `/api/user/rate` | session_id/rate/tags/comment | 成功 | 会话非 close/重复评价 |
| 8 | 用户离线留言 | POST `/api/user/offline/submit` | name/contact/content | 留言编号 | 冷却命中 |
| 9 | 客户视图聚合 | GET `/api/backend/customer/info` | user_id | 4 Tab 数据 | 部分降级 |
...
|----|------|
| 协议 | WebSocket（ws/wss） |
| 鉴权 | 由供应链系统网关统一鉴权，客服模块不重复校验；连接不合法时由网关拒绝/断开 |
...
- [x] 版本级内容只写一次，未在模块间重复；测试账号全文仅「二、测试账号」一次
- [x] 各模块 UC/AC/代号不跨模块冲突（U/W/S/C/F 全局唯一）
