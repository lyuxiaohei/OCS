...
> **范围口径**：**一期后台 11 个模块** = 侧边栏导航（数据仪表盘 / 客服工作台 / 待办事项 / 会话记录 / 会话转接 / 评价管理 / 知识文档 / 话术库 / **FAQ(🆕)** / 系统设置）+ 客户信息页（不在导航，从工作台进入，本期作为独立模块纳入）。用户端、2/3 期（工单/AI/绩效看板/技能组等）一律 Out of Scope。
...
> **V0.5 修订**（原型同步，2026-06-26）：在 V0.4 基础上同步 6/25 后改动：① 🔧 **路由规则→客服分配规则** 改名；**紧急度判定→紧急度设置** 改名；② ✅ **B13 已解决**（较急档去掉，T=首次响应超时子卡片可配）；③ ✅ **B14 已解决**（`modal-faq.html` 已建+FAQ 分类管理弹窗）；④ ➕ 自动回复区块新增 **FAQ 命中策略**（推送首条/全部/按优先级）；⑤ 🔧 排队超时从路由移至会话超时子卡片；⑥ ✅ **并发编辑需求去掉**（不再要求编辑锁定）；⑦ 🔧 设置区块名更新：客服分配规则/会话超时设置/紧急度设置/敏感词/自动回复。
>
> **V0.4 修订**（原型同步，2026-06-26）：**核对当前 `admin/*.html` 原型，修正 8 处结构性偏差**：① 🔁 **Nav 重组**：旧 3 组(工作台/服务管理/配置)→ **仪表盘(独立项) / 会话管理(组) / 知识库(组:话术·FAQ·知识文档) / 客服设置(独立项)**（`nav.js` 同步）；② 🆕 **FAQ 独立页** `admin-faq.html`（6 分类/on-off/FAQ卡片推送），从知识库页拆出为独立模块；③ ⬇️ **快捷回复再次降级**：独立页 `.deprecated` → 并入话术库为 `quickreply` 业务分类(场景=general)；话术业务分类 4→**5**；④ 🏷️ **知识库页标签改名**：nav label「知识库」→「知识文档」（实体名/文件名不变）；⑤ 🔧 **设置 6→5 区块**：欢迎语+离线语并入「自动回复(autoreply)」section；⑥ ⏱️ **超时拆 6 生命周期子阶段**：排队等待/首次响应/轮次响应/用户静默/转接/挂起，每阶段独立阈值+动作；⑦ ➕ 评价补 `skipped` 态 + 催评/一键催评；⑧ 🔖 工作台会话卡新增 `即将超时(warn)`/`转接入(transferred)` 标签。详见各模块「V0.4」标注。
>
...
| B-02 | 客服工作台 | 工作台 | `admin-workbench.html` | 三栏接待：会话列表（4 状态 Tab + 🆕 warn/transferred 标签）+ 聊天窗（收发/撤回/脱敏/富媒体卡片）+ 客户信息（4 Tab）+ 挂起/转接/结束 | P0 |
| B-03 | 待办事项 | 待办 | `admin-todo.html` | 三列集中处理：未回复消息 / 超时预警（3 类阈值）/ 待评价会话（3 态含 🆕 skipped + 催评/一键催评） | P0 |
...
| B-07 | 评价管理 | 评价 | `admin-rating-mgmt.html` | 评价监控（评分分布/排行）+ 筛选 + 差评追踪（rate≤2，🆕 入口扩到全部评价行）+ 🆕 催评转化率（skipped 态） | P0 |
| ~~B-08~~ | ~~留言管理~~ | — | ~~`admin-leave-mgmt.html`~~ | **V0.3 取消独立模块**（ADR-0012）：留言并入 B-04 会话记录的 offline 态 | — |
| B-09 | 知识文档 | 知识文档 | `admin-knowledge.html` | 知识库文章（分类树+富文本+审核闭环 5 态）。🆕 **V0.4：nav label 改「知识文档」**（实体名/文件名不变） | P1 |
| B-10 | 话术库 | 话术 | `admin-scripts.html` | 场景×业务双维度话术管理（🆕 **5 业务分类**：+quickreply 吸收快捷回复）+ 变量替换 + 关键词匹配。~~快捷回复独立页 `admin-quick-reply.html`~~ → V0.4 已 `.deprecated`，并入话术库 | P1 |
| B-11 | FAQ | FAQ | `admin-faq.html` | 🆕 **V0.4 独立页**：常见问题 Q/A 管理（6 分类/on-off 状态/触发关键词），用户端命中推送 FAQ 卡片 | P1 |
| B-12 | 系统设置 | 设置 | `admin-settings.html` | 🆕 **5 区块**：客服分配规则/会话超时设置(**6 生命周期子阶段**)/紧急度设置/敏感词/自动回复(欢迎语+离线语+FAQ命中策略)。dirty 保存 | P1 |
...
| 新建表 | OCS 新建表存储 | 知识库文章+分类（建议 `customer_chat_knowledge_base`/`kb_categories`）、FAQ（🆕 `customer_chat_faqs`+`faq_card`，表名待确认）。**留言不建表**（V0.3 并入 `customer_chat_sessions` 离线态，ADR-0012）|
...
| 账号 | 姓名 | 角色 | 用途 |
|------|------|------|------|
| 管理端测试号 | 王小明 / 张小明 / 刘洋 等 | A / S | 后台 11 模块全流程（原型内 Mock 账号；🆕 V0.4 +FAQ）|

---

## 3. 后台总览与导航结构

> 🆕 **V0.4 重组**：旧 3 组(工作台/服务管理/配置) → **2 组 + 2 独立项**。
...
> ⚠️ 客户信息不在侧边栏导航，从工作台进入（见 B-05 说明）。
> ⬇️ 快捷回复独立页 `admin-quick-reply.html` → V0.4 已 `.deprecated`，并入话术库 `quickreply` 业务分类。
...
> 🆕 **V0.4**：nav label 改「知识文档」（原名「知识库」，实体名/文件名 `admin-knowledge.html` 不变）；快捷回复已 `.deprecated` 并入话术库；FAQ 拆为独立模块。
...
> 🆕 **V0.4 新增独立模块**：`admin-faq.html`（独立页，面包屑「知识库 / FAQ」）。维护常见问题 Q/A，供移动端自动发送 FAQ 卡片：用户咨询命中关键词或进入自助时，系统以卡片形式推送对应 FAQ。
...
| UC-FAQ-02 | S | 新建 FAQ | 点「新建 FAQ」→ 填写问题/答案/分类/触发关键词 | 📝 **待补**：新建编辑表单未原型化 |
| UC-FAQ-03 | S | 编辑 FAQ | 行「编辑」 | 📝 同新建，表单未原型化 |
...
| FAQ 分类 faq-category | order/return/pay/logistic/account/other | 🆕 6 分类（原型标注，待确认最终值）|
| FAQ 触发关键词 | 自由标签 | 用户消息命中匹配 |

> 数据来源：⚠️ **待建表** `customer_chat_faqs`+`faq_card`（原型标注，表名/字段待确认）。`faq_card` 为消息类型扩展——用户端收到 FAQ 卡片消息。
...
| 业务 biz | presale(售前)/aftersale(售后)/complaint(投诉)/general(通用)/**quickreply(快捷回复,🆕V0.4)** | 5 类（🆕 quickreply 吸收已废弃的快捷回复独立页） |

> 🆕 **V0.4**：快捷回复独立页 `admin-quick-reply.html` → `.deprecated`，并入话术库为 `quickreply` 业务分类（场景=general）。~~旧"两套分类勿混用"警示已失效~~——快捷回复不再独立存在。
...
| 业务 scene-badge | biz-presale/aftersale/complaint/general/quickreply | 🆕 5 类（+quickreply 吸收快捷回复）|
| 启用状态 | on/off | 新建默认启用 |
...
> 🆕 **V0.4**：**5 区块**（客服分配规则/会话超时设置/紧急度设置/敏感词/自动回复）。欢迎语+离线语并入「自动回复」section；会话超时拆为 6 生命周期子阶段。🔧 **V0.5**：路由改名"客服分配规则"，紧急度改名"紧急度设置"，自动回复新增 FAQ 命中策略。
...
### 15.2.1 五区块配置项（🆕 V0.4：真实字段名/默认值）

> 🆕 **V0.4 重组**：欢迎语+离线语并入「自动回复 #autoreply」；客服分配规则仅轮询/空闲度（无 VIP/技能组）；超时拆 6 生命周期子阶段。
...
| 超时 | 🆕 排队等待超时 | input | **4** 分钟 | wait 阶段；超过未接入→预警 |
| 超时 | 🆕 首次响应超时 | input | **5** 分钟 | accept 首轮；超过→提醒客服 |
| 超时 | 🆕 轮次响应超时 | input | **3** 分钟 | accept 对话轮次；超过→提醒客服 |
| 超时 | 用户静默超时 `idle_timeout` | input | **15** 分钟 | 客服最后回复后计时（对应旧 idle_timeout）|
| 超时 | 自动关闭时长 `auto_close_timeout` | input | **30** 分钟 | 静默超时后无回复→自动关闭 |
| 超时 | 🆕 转接超时 | input | **5** 分钟 | transfer 阶段 TTL；超时→回退 |
| 超时 | 🆕 挂起超时 | input | **10** 分钟 | pending 阶段；超时→自动恢复 |
...
| 自动回复 | 🆕 欢迎语 `welcome_content` | textarea | "您好，欢迎联系商城客服中心！…" | ≤200 字；进入会话立即推送 |
| 自动回复 | 🆕 离线语 `offline_content` | textarea | "当前无客服在线（工作时间 9:00-21:00）…" | ≤200 字 |
| 自动回复 | 允许留言 | switch | 开 | 留言进入会话 offline 态（ADR-0012）|
| 自动回复 | 离线会话通知方式 | checkbox | 站内消息 + 企微推送客服 | 短信通知客服（置灰，暂不支持）|
| 自动回复 | 🆕 FAQ 命中策略 | radio 3 | 推送首条 | 命中多个 FAQ 时：推送首条 / 推送全部 / 按优先级 |

> **紧急度规则**见 [待办.2.4](#待办24-紧急度判定规则)：等待 ≥ 首次响应阈值 T（=首次响应超时子卡片，**✅ V0.5 已可配**，B13 已解决）或命中紧急关键词 → 紧急；否则一般。
...
- [x] 范围聚焦后台 11 模块（🆕 V0.4 +FAQ），用户端/2-3 期明确 Out of Scope
...
- 🔁 **Nav 重组**：旧 3 组(工作台/服务管理/配置) → 仪表盘(独立项) / 会话管理(组) / 知识库(组:话术·FAQ·知识文档) / 客服设置(独立项)
- 🆕 **FAQ 独立页** `admin-faq.html`：6 分类/on-off/FAQ 卡片推送，从知识库页拆出
- ⬇️ **快捷回复再次降级**：独立页 `.deprecated` → 并入话术库 `quickreply` 业务分类；话术业务分类 4→5
- 🏷️ **知识库页标签改名**：nav label「知识库」→「知识文档」（实体名/文件名不变）
- 🔧 **设置 6→5 区块**：欢迎语+离线语并入「自动回复(autoreply)」section
- ⏱️ **超时拆 6 生命周期子阶段**：排队等待/首次响应/轮次响应/用户静默/转接/挂起
- ➕ 评价补 `skipped` 态 + 催评/一键催评
- 🔖 工作台会话卡新增 `即将超时(warn)`/`转接入(transferred)` 标签
