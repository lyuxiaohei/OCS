...
| 消息提醒机制（V2.2）| 新消息到达 -> **弹窗 + 提示音**双通道（会议硬性要求，参考四川气鱼）；**超时未回复**亦触发提示音（视觉见待办超时预警）。**当前焦点会话仅徽标 +1 不弹窗**（免打扰）；工作台非激活页签/最小化时弹窗+声音必达。[待补] **待补**：提示音开关/音量是否进系统设置属增量，一期默认开。 |
...
> 以下为本文档完整修订历史（V2.1 -> V0.4），按时间倒序排列。正文已融入各版本决策，此处仅作溯源。
