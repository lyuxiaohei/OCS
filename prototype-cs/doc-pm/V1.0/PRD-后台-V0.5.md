...
- [1. 范围边界](#1-范围边界)
  ...
  - [1.3 实现来源口径](#13-实现来源口径两维度四值)
- [2. 角色列表](#2-角色列表)
  - [2.1 测试账号](#21-测试账号)
- [3. 后台总览与导航结构](#3-后台总览与导航结构)
- [4. 数据仪表盘](#4-数据仪表盘)
- [5. 客服工作台](#5-客服工作台)
- [6. 待办事项](#6-待办事项)
- [7. 会话记录](#7-会话记录)
...
- [9. 会话转接](#9-会话转接)
- [10. 评价管理](#10-评价管理)
- [11. 留言管理（已并入）](#11-留言管理v03-起已并入会话记录)
- [12. 知识文档](#12-知识文档)
- [13. FAQ](#13-faq)
- [14. 话术库](#14-话术库)
- [15. 系统设置](#15-系统设置)
...
| B-07 | 评价管理 | 评价 | `admin-rating-mgmt.html` | 评价监控（评分分布/排行）+ 筛选 + 差评追踪（rate≤2，[新增] 入口扩到全部评价行）+ [新增] 催评转化率（skipped 态） | P0 |
| ~~B-08~~ | ~~留言管理~~ | — | ~~`admin-leave-mgmt.html`~~ | **V0.3 取消独立模块**（ADR-0012）：留言并入 B-04 会话记录的 offline 态 | — |
| B-09 | 知识文档 | 知识文档 | `admin-knowledge.html` | 知识库文章（分类树+富文本+审核闭环 5 态）。[新增] **V0.4：nav label 改「知识文档」**（实体名/文件名不变） | P1 |
| B-10 | 话术库 | 话术 | `admin-scripts.html` | 场景×业务双维度话术管理（[新增] **5 业务分类**：+quickreply 吸收快捷回复）+ 变量替换 + 关键词匹配。~~快捷回复独立页 `admin-quick-reply.html`~~ -> V0.4 已 `.deprecated`，并入话术库 | P1 |
| B-11 | FAQ | FAQ | `admin-faq.html` | [新增] **V0.4 独立页**：常见问题 Q/A 管理（6 分类/on-off 状态/触发关键词），用户端命中推送 FAQ 卡片 | P1 |
| B-12 | 系统设置 | 设置 | `admin-settings.html` | [新增] **5 区块**：客服分配规则/会话超时设置(**6 生命周期子阶段**)/紧急度设置/敏感词/自动回复(欢迎语+离线语+FAQ命中策略)。dirty 保存 | P1 |
...
> 仪表盘「今日概况」原型的「AI 托管会话 142 / 11%」为 Mock 占位，**一期 source 无 AI（3 期）-> 一期隐藏该指标位、今日概况由 6 项减为 5 项**，3 期 AI 上线时加回（见 §4.3）。

### 1.3 实现来源口径（两维度四值）

各模块数据的「落点 × 工作量」按四值分类，在 §17 接口时序与各模块字段表中以「来源」列标注：

| 实现来源 | 含义 | 本期典型 |
|---|---|---|
| 复用 | OCS 已有表/接口直接用 | 会话/消息/转接/评价(rate)（`customer_chat_sessions`/`messages`/`transfers`）|
| 新建表 | OCS 新建表存储 | 知识库文章+分类（建议 `customer_chat_knowledge_base`/`kb_categories`）、FAQ（[新增] `customer_chat_faqs`+`faq_card`，表名待确认）。**留言不建表**（V0.3 并入 `customer_chat_sessions` 离线态，ADR-0012）|
...
| 账号 | 姓名 | 角色 | 用途 |
|------|------|------|------|
| 管理端测试号 | 王小明 / 张小明 / 刘洋 等 | A / S | 后台 11 模块全流程（原型内 Mock 账号；[新增] V0.4 +FAQ）|

---

## 3. 后台总览与导航结构

> [新增] **V0.4 重组**：旧 3 组(工作台/服务管理/配置) -> **2 组 + 2 独立项**。

侧边栏由 `nav.js` 统一渲染（`#sidebar-nav` + `data-active`），`MENU_DATA` 定义结构：

```
数据仪表盘（独立项）

会话管理（分组）  -> 客服工作台  待办事项  会话记录  会话转接  评价管理
...
> [注意] 客户信息不在侧边栏导航，从工作台进入（见 B-05 说明）。
> [降级] 快捷回复独立页 `admin-quick-reply.html` -> V0.4 已 `.deprecated`，并入话术库 `quickreply` 业务分类。
...
flowchart LR
    ...
    评价 -->|查看会话| 工作台
    话术 -->|"话术推荐(含 quickreply 快捷回复)"| 工作台
    知识文档 -.->|知识库文章引用| 工作台
    FAQ -.->|FAQ 卡片推送| 工作台
    设置 -.->|"客服分配规则/超时/敏感词/自动回复全局生效"| 工作台
```

> **关键约定**：客服工作台为常驻全屏工作页签——侧边栏默认收起（`.admin-sidebar` `translateX(-208px)`，由 `.sidebar-trigger` 浮层展开），在工作台内点任何菜单都新开页签，不离开工作台。其余页侧边栏常驻。
...
> [待补] **待补**：原型为静态写死（Mock），无真实接口调用。下表字段为 PRD 落库口径（原型未显式声明接口），供后端实现。
...
> [待补] **待补**：原型未给排队->接入的实时刷新策略、断连/空数据降级 UI。建议后端补 WS 推送刷新 + 超时降级占位（与 V1.0 仪表盘超时降级约定一致）。
...
| UC-工作台-15 | A | 新消息提醒（V2.2）| 非焦点会话收到用户新消息 | **弹窗提示（用户名+消息预览+「立即查看」）+ 提示音**；会话项未读徽标 +1。会议硬性要求「声音+弹窗」双通道（参考四川气鱼），防止客服页面多/接电话时遗漏。**多条消息堆叠展示（最多 3 条同时可见），超出折叠为「还有 N 条未读消息」药丸，点击展开全部；持久模式（`persist:true`）不自动消失，仅手动关闭**。弹窗名称为**纯用户名**（如「林小娟」），非「新用户 · 林小娟」；转接类弹窗正文为**结构化两行**（转接原因/转接备注），非单行长文本 |
...
| AC-工作台-10 | UC-工作台-15 | 非焦点会话 | WS `receive-message` 到达 | 弹消息提醒弹窗 + 播放提示音；**当前焦点会话仅徽标 +1 不弹窗**（免打扰）；工作台处于非激活页签/窗口最小化时弹窗+声音必达。**多条弹窗堆叠展示（最多 3 条可见，`MAX_VISIBLE=3`），超出折叠为顶部「🔔 还有 N 条未读消息，点击展开」药丸（`.nm-pill`），点击展开全部（容器 `.nm-popup-stack.expanded` 可滚动）；关闭一条后自动从折叠区补一条进来保持 3 条可见；全部清空时重置展开态** |
...
| 场景 | 处理 |
|------|------|
| 撤回时间戳缺省 | [待补] **待补口径**：原型注释「缺省视为已超时」，但代码 `if(sentTs)` 实为缺省放行。**本稿统一约定：无时间戳视为已超时，拒绝撤回**（以注释口径为准），后端实现须保证消息落库时带 `sent_ts`。 |
| 行为/交易/服务三栏不联动 | 原型右栏仅「基础信息」随客户切换刷新，行为/交易/服务恒显示默认客户 Mock。[待补] **待补**：若要求四栏全联动，属增量需求。 |
...
| 消息提醒机制（V2.2）| 新消息到达 -> **弹窗 + 提示音**双通道（会议硬性要求，参考四川气鱼）；**超时未回复**亦触发提示音（视觉见待办超时预警）。**当前焦点会话仅徽标 +1 不弹窗**（免打扰）；工作台非激活页签/最小化时弹窗+声音必达。**多条消息堆叠规则（V2.2 修订）**：最多 3 条弹窗同时可见（`.nm-popup-stack` `MAX_VISIBLE=3`），第 4 条到达时最旧一条折叠（不删除，`.nm-collapsed`），顶部出现「🔔 还有 N 条未读消息，点击展开」药丸（`.nm-pill`）；点击药丸展开全部（`.expanded`，容器可滚动）；关闭一条后自动从折叠区补一条保持 3 条可见；全部清空重置展开态。**持久模式（`persist:true`）**：4 类模拟场景（新接入/新转接/转接被拒/转接超时）弹窗不自动消失，仅「立即查看」/「✕」/外部 `close()` 可关闭。**弹窗名称规范**：纯用户名（如「林小娟」），非「新用户 · 林小娟」；转接类弹窗正文为**结构化两行**（`.nm-lines`：转接原因 + 转接备注），非单行长文本。[待补] **待补**：提示音开关/音量是否进系统设置属增量，一期默认开。 |
| 三栏布局宽度（V2.2 修订）| 工作台 shell `.wb-shell` 限宽 `min-width:1200px / max-width:1280px` 且水平居中（宽屏不拉散）；左栏会话列表固定 `320px`；**中栏聊天窗固定 `480px`（较窄、扁平紧凑，避免宽屏下过宽）**；右栏客户信息 `flex:1`（`min-width:320px / max-width:480px`，吸收中栏收窄后剩余空间）。 |

## 5.3 数据流向

> [待补] **待补**：原型工作台**未出现任何 `/api/backend/...` 接口路径，也未出现 WS action**，发送消息脚本注释明确 `// no real backend`。下表为 PRD 基于视觉规格定义的接口契约，非还原已有。
...
> 阈值与 [模块 · 系统设置](#模块--系统设置) 系统设置联动：`idle_timeout`(静默)/首次响应/排队/`queue_timeout`。原型为写死文案，[待补] **待补**：阈值应改为读取系统设置。
>
...
| 角色权限（ADR-0002）| **A 可见**：未回复回复、超时接入/转接、待评价筛选；**S 主管级**：超时**催办**、待评价**催评/一键催评全部**、规则配置（UC-待办-03/06 角色=S）。 |

### 6.2.4 紧急度设置

紧急度（`urgency` = high/low，即 紧急/一般）决定**未回复消息列**的行着色与排序。一期由**系统按规则自动判定**，客服不可手动标记（手标/覆盖留 2 期）。

**判定逻辑**：命中下述任一即 **紧急**，否则 **一般**：
- 等待时长 ≥ 首次响应阈值 T（响应超时，见 待办.2.1，默认 5 分钟）；或
- 会话/消息命中**紧急类目关键词组**（退款/退货/投诉/差评/破损/物流异常，钱损·投诉风险，不论等待时长）。

> **类目关键词匹配**复用后端规则引擎（`controller/rule`，全匹配/部分匹配）；关键词组在 [系统设置 · 紧急度](#设置21-五区块配置项真实字段名默认值) 维护。**V0.3 起仅一组**——原「较急类目关键词组」已移除，紧急度收敛为 紧急/一般 两档。
>
> **与「超时预警」列的关系**：未回复列的紧急（时间梯度）= 临超时，目的是在变成超时预警列之前抢先处理；两列不重复——超时列 = 已破阈值，未回复列紧急 = 即将破阈值。
>
> **反向验证**：本规则可解释现有 demo 数据——李婷婷「退款金额不对」=紧急（类目）、张伟「商品破损换货」=紧急（类目）、刘洋「物流没更新」=紧急（物流异常类目）、陈晓「优惠券无效」/周芳/吴磊/郑丽/孙强（积分/发货/发票/货到付款）=一般。
...
| 字段 | 类型 | 取值 | 说明 |
|------|------|------|------|
| 紧急度 urgency | enum | high/low（紧急/一般） | 决定未回复列着色与排序；系统按 [待办.2.4](#待办24-紧急度判定规则) 规则自动判定（等待≥T 或 命中紧急关键词 -> 紧急，V0.3 收敛为两档），一期客服不可手标 |
...
> [待补] **V0.3 与 V2.2 差异**：原「历史会话」+「留言管理」两模块/两菜单合并为「会话记录」单一模块/菜单；留言收敛为会话「离线待处理」状态（ADR-0012）。原留言独立 4 态机（pending/process/replied/invalid）与"分配/回复/标无效"动作并入会话：pending->会话 offline 态、分配->会话路由分配、回复->会话消息收发、invalid->会话关闭子态。
...
| 行为轨迹 | 埋点系统 | `GET /api/user/behavior` | [待补] **2 期：一期灰显占位「即将上线」**（埋点源最不稳，踢出关键路径，ADR-0010）|
| 交易记录 | OMS/订单库 | `GET /api/user/transactions`（与工作台订单卡共享）| [待补] Mock 横幅 |
...
| 降级粒度 | 按源独立降级，**未做**「全失败仅留基础信息」全局兜底。[待补] **待补**：如需全局兜底属增量。 |
| 超时阈值 | 原型仅文案「响应超时」，无具体毫秒数。[待补] **待补**：建议约定（如 2s）。 |
...
| AC-转接-03 | UC-转接-05 | 顶部 | 点发起转接 -> 确认 | `addPendingRow`：表头插黄底行 + 状态「待处理 [超时] 剩余 5:00」+ 原因映射 + Tab/Stat/PgInfo 计数 +1 |
| AC-转接-04 | UC-转接-08 | 待处理行 | 点取消 | 状态格->`cancelled` 已取消 + `data-status=cancelled` + 操作仅「详情」+ 去黄底 |
| AC-转接-05 | UC-转接-09 | 超时行 | 点重新转接 -> 确认 | `setRowPending`：状态回「待处理 + [超时]剩余 5:00」+ 目标更新 + 操作恢复「催办/详情/取消」+ 黄底 |
...
| 容量上限 | 原型硬编码 10，阈值 50%/100% 写死。[待补] **待补**：应参数化为系统设置 `max_sessions`。 |
| TTL 倒计时 | 原型为静态文本「[超时] 剩余 3:42」，非真实计时。[待补] **待补**：实时倒计时属增量。 |
...
> [待补] **与 V1.0 差异（必须修正）**：V1.0 评价写了「attitude/speed/professional/solution 四维评分各 1-5」。**原型只有单一星值（1-5）+ 标签集合，未渲染四维分数**。本期评价为单一维度；四维属后端字段预留，前端不展示。
...
> - 留言 4 态机 -> 收敛进会话状态机（见 §16.1，新增 offline 态）
>
> 原留言单据（§16.3）、`leave/assign`·`leave/reply` 接口（§17.1 #5/#6）同步废止。"回复回灌下次会话"（ADR-0004）在统一模型下自然成立——同一会话线程内，客服上线接入后即回复。

---

# 12. 知识文档

> [新增] **V0.4**：nav label 改「知识文档」（原名「知识库」，实体名/文件名 `admin-knowledge.html` 不变）；快捷回复已 `.deprecated` 并入话术库；FAQ 拆为独立模块。

知识库文章管理：分类树（5 类）+ 富文本编辑器 + 审核闭环（5 态）+ 导入文档 + 浏览量统计。

## 12.1 功能用例与验收

| 编号 | 角色 | 场景 | 动作 | 预期结果 |
|------|------|------|------|----------|
| UC-KB-01 | S | 分类树筛选 | 点分类节点 | 节点高亮 + 同步下拉 + 右表过滤 |
| UC-KB-02 | S | 新建/编辑文章 | 点新建/编辑 | 富文本编辑器展开 |
| UC-KB-03 | S | 审核文章 | 待审核行「审核」-> 通过/驳回 | 通过->已发布；驳回->已驳回 + 原因 |
| UC-KB-04 | S | 上下线 | 已发布「下线」/已下线「上线」 | 状态互换 |
| UC-KB-05 | S | 删除文章 | 行「删除」 | 二次确认弹窗（不可恢复）|

### 12.1.1 验收标准

| 编号 | 对应用例 | Given | When | Then |
|------|:------:|-------|------|------|
| AC-KB-01 | UC-KB-03 | 待审核行 | 点审核 -> 驳回（不填原因）| 驳回原因必填；提交通过->`s-tag published`；驳回->`s-tag rejected` |
| AC-KB-02 | UC-KB-01 | 分类树 | 点节点 | `.tree-node.active` + 同步 `#kbCategory` + toast「已筛选分类：X」|
| AC-KB-03 | UC-KB-05 | 任意行 | 点删除 | `modal-delete-confirm`，desc「文章《X》删除后不可恢复」|
...
> 数据来源：**新建表**（建议表名，当前后端未建）`customer_chat_knowledge_base`（文章）+ `customer_chat_kb_categories`（分类）。已发布文章->用户端知识库。

---

# 13. FAQ

> [新增] **V0.4 新增独立模块**：`admin-faq.html`（独立页，面包屑「知识库 / FAQ」）。维护常见问题 Q/A，供移动端自动发送 FAQ 卡片：用户咨询命中关键词或进入自助时，系统以卡片形式推送对应 FAQ。

## 13.1 功能用例与验收

| 编号 | 角色 | 场景 | 动作 | 预期结果 |
|------|------|------|------|----------|
| UC-FAQ-01 | S | 筛选 FAQ | 选择分类/状态/搜索 | 表格过滤刷新 |
| UC-FAQ-02 | S | 新建 FAQ | 点「新建 FAQ」-> 填写问题/答案/分类/触发关键词 | [待补] **待补**：新建编辑表单未原型化 |
| UC-FAQ-03 | S | 编辑 FAQ | 行「编辑」 | [待补] 同新建，表单未原型化 |
| UC-FAQ-04 | S | 启停 FAQ | 行「启用」/「停用」 | 状态 on<->off 切换 |
| UC-FAQ-05 | S | 删除 FAQ | 行「删除」 | 二次确认弹窗 |

### 13.1.1 验收标准

| 编号 | 对应用例 | Given | When | Then |
|------|:------:|-------|------|------|
| AC-FAQ-01 | UC-FAQ-01 | FAQ 列表 | 切换分类下拉/状态下拉/输入搜索词 | 表格实时过滤 |
| AC-FAQ-02 | UC-FAQ-04 | 启用态 FAQ | 点「停用」 | 状态变 off(灰点)，不再参与匹配推送 |
| AC-FAQ-03 | UC-FAQ-04 | 停用态 FAQ | 点「启用」 | 状态变 on(绿点)，恢复匹配推送 |
...
|------|------|
| 删除 | 二次确认（不可恢复） |
| 同关键词多 FAQ | [待补] **待补**：命中多个时推送策略（首条/全部/按优先级）|
...
| FAQ 分类 faq-category | order/return/pay/logistic/account/other | [新增] 6 分类（原型标注，待确认最终值）|
| FAQ 触发关键词 | 自由标签 | 用户消息命中匹配 |

> 数据来源：[注意] **待建表** `customer_chat_faqs`+`faq_card`（原型标注，表名/字段待确认）。`faq_card` 为消息类型扩展——用户端收到 FAQ 卡片消息。
...
| 业务 biz | presale(售前)/aftersale(售后)/complaint(投诉)/general(通用)/**quickreply(快捷回复,[新增]V0.4)** | 5 类（[新增] quickreply 吸收已废弃的快捷回复独立页） |

> [新增] **V0.4**：快捷回复独立页 `admin-quick-reply.html` -> `.deprecated`，并入话术库为 `quickreply` 业务分类（场景=general）。~~旧"两套分类勿混用"警示已失效~~——快捷回复不再独立存在。
...
| 业务 scene-badge | biz-presale/aftersale/complaint/general/quickreply | [新增] 5 类（+quickreply 吸收快捷回复）|
| 启用状态 | on/off | 新建默认启用 |
...
> [新增] **V0.4**：**5 区块**（客服分配规则/会话超时设置/紧急度设置/敏感词/自动回复）。欢迎语+离线语并入「自动回复」section；会话超时拆为 6 生命周期子阶段。[修改] **V0.5**：路由改名"客服分配规则"，紧急度改名"紧急度设置"，自动回复新增 FAQ 命中策略。
...
| UC-设置-02 | S | 编辑配置 | 改任意配置项 | 区块加 `.dirty`（左 3px 橙竖条）+ 底部「[注意] 有 N 个模块已修改」 |
...
| 编号 | 对应用例 | Given | When | Then |
|------|:------:|-------|------|------|
| AC-设置-01 | UC-设置-02 | 任意区块 | 改 textarea/input/switch/radio | `markDirty(section)`：`.card.setting-section.dirty`（左 3px `--cs-warning` 竖条）+ `#saveHint`「[注意] 有 N 个模块已修改」 |
...
### 15.2.1 五区块配置项（[新增] V0.4：真实字段名/默认值）

> [新增] **V0.4 重组**：欢迎语+离线语并入「自动回复 #autoreply」；客服分配规则仅轮询/空闲度（无 VIP/技能组）；超时拆 6 生命周期子阶段。

| 区块 | 字段 | 控件 | 默认值 | 说明 |
|------|------|------|--------|------|
| 客服分配规则 | `is_auto_accept` | switch | 开（自动接入）| 新会话按分流策略自动分配 |
| 客服分配规则 | 分流策略 `routing_strategy` | radio 2 | 轮询分配（均衡）| 按空闲度（无 VIP/技能组，属 2-3 期）|
| 客服分配规则 | 单客服接待上限 `max_sessions` | input | **10** 个会话 | 达上限进排队 |
| 客服分配规则 | 关键词转人工 `transfer_keywords` | switch + 词表 | 开；词：人工/客服/投诉/退款/售后/转人工 | 命中跳过机器人 |
| 超时 | [新增] 排队等待超时 | input | **4** 分钟 | wait 阶段；超过未接入->预警 |
| 超时 | [新增] 首次响应超时 | input | **5** 分钟 | accept 首轮；超过->提醒客服 |
| 超时 | [新增] 轮次响应超时 | input | **3** 分钟 | accept 对话轮次；超过->提醒客服 |
| 超时 | 用户静默超时 `idle_timeout` | input | **15** 分钟 | 客服最后回复后计时（对应旧 idle_timeout）|
| 超时 | 自动关闭时长 `auto_close_timeout` | input | **30** 分钟 | 静默超时后无回复->自动关闭 |
| 超时 | [新增] 转接超时 | input | **5** 分钟 | transfer 阶段 TTL；超时->回退 |
| 超时 | [新增] 挂起超时 | input | **10** 分钟 | pending 阶段；超时->自动恢复 |
| 超时 | 超时提醒文案 | textarea | "您已长时间未回复…" | — |
| 超时 | 关闭提醒文案 | textarea | "由于您长时间未回复，本次会话已关闭…" | 关闭时推送 |
| 超时 | 关闭后允许重发 | switch | 关（默认 off）| 关闭后可再次咨询 |
| 紧急度设置 | 紧急类目关键词组 `urgency_high_keywords` | 词表（可增删） | 退款/退货/投诉/差评/破损/物流异常 | 命中 -> 紧急（钱损/投诉风险，不论等待时长）|
| 敏感词 | 全局过滤开关 | switch | 开 | **词库由商城供给**（ADR-0007，OCS 不自建）|
| 敏感词 | 命中策略 `sensitive_strategy` | radio 2 | 替换为 ***（双方） | 另一项「仅用户端替换 ***」 |
| 自动回复 | [新增] 欢迎语 `welcome_content` | textarea | "您好，欢迎联系商城客服中心！…" | ≤200 字；进入会话立即推送 |
| 自动回复 | [新增] 离线语 `offline_content` | textarea | "当前无客服在线（工作时间 9:00-21:00）…" | ≤200 字 |
| 自动回复 | 允许留言 | switch | 开 | 留言进入会话 offline 态（ADR-0012）|
| 自动回复 | 离线会话通知方式 | checkbox | 站内消息 + 企微推送客服 | 短信通知客服（置灰，暂不支持）|
| 自动回复 | [新增] FAQ 命中策略 | radio 3 | 推送首条 | 命中多个 FAQ 时：推送首条 / 推送全部 / 按优先级 |

> **紧急度规则**见 [待办.2.4](#待办24-紧急度判定规则)：等待 ≥ 首次响应阈值 T（=首次响应超时子卡片，**[已解决] V0.5 已可配**，B13 已解决）或命中紧急关键词 -> 紧急；否则一般。
...
> [待补] **与 V1.0 差异（必须修正）**：V1.0 系统设置含 `pending_resume_default`（挂起恢复默认 30 分钟）。**原型系统设置页无此项**——挂起恢复时间在工作台挂起弹窗（15/30/60 默认 30）体现，不属系统设置配置项。若需可配，属增量。~~敏感词库为新建表 `customer_chat_sensitive_words`~~ **已作废**：词库改由商城供给、OCS 不自建（ADR-0007）。
...
| max_sessions / queue_timeout / idle_timeout / auto_close_timeout / routing_strategy / sensitive_strategy / transfer_keywords | — | [待补]落表字段（底部原文仅点名前三列）|
...
### 16.1 会话单（customer_chat_sessions）
...
### 16.2 转接单（customer_chat_transfers）
...
### 16.3 留言单（V0.3 取消，并入会话单离线态）

> **V0.3 起取消独立留言单**（ADR-0012）：留言为会话 `offline` 离线态（见 §16.1），不再有独立 `#LM` 单据与 pending/process/replied/invalid 状态机。原留言字段（content/contact/问题分类）落入会话/消息；"回复回灌"统一为客服上线接入后在同一会话线程内回复。`leave/assign`·`leave/reply` 接口废止（见 §17.1）。
...
> [待补] 接口路径中 `/api/user/transactions`、`/api/user/behavior` 为原型声明/约定的契约，其余 `/api/backend/*` 为 V1.0 继承 + 原型佐证的后台口径。原型 HTML 本身未硬编码路径（均为 Mock/占位）。**V0.3：`/api/backend/leave/*` 废止**（留言并入会话离线态，ADR-0012），离线态分配复用 `session/accept`。
...
- [x] 范围聚焦后台 11 模块（[新增] V0.4 +FAQ），用户端/2-3 期明确 Out of Scope
- [x] 功能清单以 admin 原型为准（逐页提取，非转述 V1.0；V0.4 同步 nav 重组 + FAQ 独立页 + 快捷回复降级）
- [x] 每个 UC 的 GWT 可独立测试，UC<->AC 编号完整
- [x] 已标注所有「V1.0 与原型不符」的修正点
- [x] 涉及数据字段标「来源/去向」；统计口径写入字段表
- [x] 凡原型未实现但 PRD 需约束的点统一「[待补]」标注，不冒充原型既有
- [x] 客户信息页作为独立模块（客户信息）纳入，与工作台右栏内联视图区分
- [x] 各模块 UC/AC/简称全局唯一（仪表盘/工作台/待办/会话记录/客户信息/转接/评价/知识文档/话术/FAQ/设置）
...
- [x] **V0.4（原型同步修订，2026-06-26）**：① Nav 重组 2 组+2 独立项；② FAQ 独立模块；③ 快捷回复降级并入话术库；④ 知识库->知识文档；⑤ 设置 6->5 区块；⑥ 超时 6 生命周期子阶段；⑦ 评价 skipped+催评；⑧ 工作台 warn/transferred 标签。
- [x] **V0.5 修订①（原型同步，2026-07-07）**：① 新消息弹窗由「单实例替换」改为「堆叠 + 折叠」（`MAX_VISIBLE=3`，超出折叠为「🔔 还有 N 条未读消息」药丸，点击展开全部，不丢消息）；② 4 类场景弹窗改 `persist:true`（不自动消失，仅手动关闭）；③ 弹窗名称由「新用户 · 林小娟」改为纯用户名「林小娟」；④ 转接弹窗正文由单行 `preview` 改为结构化两行 `lines`（转接原因 + 转接备注）；⑤ 工作台三栏宽度调整：中栏聊天窗固定 `480px`（较窄扁平），右栏客户信息 `flex:1` 吸收剩余空间，shell 限宽 `1280px` 居中。


## 附录A. 修订记录

> 以下为本文档完整修订历史（V2.1 -> V0.5 修订①），按时间倒序排列。正文已融入各版本决策，此处仅作溯源。

### A.0 V0.5 修订①（原型同步，2026-07-07）

核对 `admin-workbench.html` + `admin/js/notify.js` 原型修订，同步 5 处：
- [堆叠] **新消息弹窗堆叠 + 折叠**：由单实例（新弹窗直接删除旧的）改为多实例堆叠（`.nm-popup-stack`，`flex-direction:column-reverse`），最多 3 条同时可见（`MAX_VISIBLE=3`）；第 4 条到达时最旧一条折叠（`.nm-collapsed`，`display:none`，不删除），顶部出现「🔔 还有 N 条未读消息，点击展开」药丸（`.nm-pill`）；点击药丸展开全部（`.nm-popup-stack.expanded`，容器 `max-height + overflow-y:auto` 可滚动）；关闭一条后自动从折叠区补一条保持 3 条可见；全部清空重置展开态。对应 `notify.js` `popup()/collapseOverflow()/updatePill()/expandAll()`。
- [持久] **4 类模拟场景弹窗 `persist:true`**：新接入/新转接/转接被拒/转接超时 4 个弹窗不启动 5 秒自动消失定时器，仅「立即查看」/「✕」/外部 `close()` 可关闭。避免客服未及时查看即丢失提醒。
- [名称] **弹窗名称改纯用户名**：「新用户 · 林小娟」->「林小娟」；「转接 · 王志强（来自 李静）」->「王志强（来自 李静）」；「转接 · 王志强 拒绝了您的转接」->「王志强 拒绝了您的转接」；「转接超时 · 王志强 未在 5 分钟内响应」->「王志强 未响应转接」。头像取 `name.charAt(0)` 显示姓氏（如「林」「王」）。
- [转接正文] **转接弹窗改结构化两行**：由单行 `preview`「（转接）订单 #20260611039 退款问题，需售后组跟进」改为 `lines: [{label:'转接原因', value:'订单 #20260611039 退款问题'}, {label:'转接备注', value:'需售后组跟进'}]`，渲染为 `.nm-lines` 两行（标签灰、值深色）。`notify.js` `popup()` 新增 `lines` 参数支持，与 `preview` 二选一。
- [布局] **三栏宽度调整**：`.wb-shell` 增 `max-width:1280px; margin:0 auto`（宽屏居中）；`.wb-middle` 由 `flex:1`（占满剩余）改为 `width:480px; flex-shrink:0`（固定较窄，聊天窗扁平紧凑）；`.wb-right` 由 `width:320px; flex-shrink:0` 改为 `flex:1; min-width:320px; max-width:480px`（吸收中栏收窄后剩余空间，客户信息区更宽）。

### A.1 V0.4（原型同步，2026-06-26）

核对当前 `admin/*.html` 原型，修正 8 处结构性偏差：
- [重组] **Nav 重组**：旧 3 组(工作台/服务管理/配置) -> 仪表盘(独立项) / 会话管理(组) / 知识库(组:话术·FAQ·知识文档) / 客服设置(独立项)
- [新增] **FAQ 独立页** `admin-faq.html`：6 分类/on-off/FAQ 卡片推送，从知识库页拆出
- [降级] **快捷回复再次降级**：独立页 `.deprecated` -> 并入话术库 `quickreply` 业务分类；话术业务分类 4->5
- [标签] **知识库页标签改名**：nav label「知识库」->「知识文档」（实体名/文件名不变）
- [修改] **设置 6->5 区块**：欢迎语+离线语并入「自动回复(autoreply)」section
- [超时] **超时拆 6 生命周期子阶段**：排队等待/首次响应/轮次响应/用户静默/转接/挂起
- [增加] 评价补 `skipped` 态 + 催评/一键催评
- [标记] 工作台会话卡新增 `即将超时(warn)`/`转接入(transferred)` 标签

### A.2 V0.3（会话/留言统一模型，ADR-0012，2026-06-24）

- 取消「留言」独立实体/表/菜单——留言为会话的「离线待处理(offline)」态
- 「历史会话」+「留言管理」合并为单一「会话记录」模块（11->10 模块）
- 快捷回复拆为独立导航页 `admin-quick-reply.html`（9->10 导航页）
- 设置紧急度区块原型已补（5->6 区块）

### A.3 V2.2（初步沟通2 会议决策，2026-06-22）

- ① 工作台消息提醒机制（弹窗+提示音）+ 超时声音提醒
- ② 历史会话「按用户归集」视图 +「再次联系」入口
- ③ 渠道标识 2 层模型（父品牌商城/子应用，一期仅显 H5）
- ④ 数据可见性按角色隔离（A 仅见自己的会话、S 见全部）
- ⑤ 富媒体卡片补 cart_card / logistic_card / refund_card
- 会议明确不改：敏感词维持 ADR-0007、路由默认维持轮询

### A.4 V2.1（grill-with-docs 产品架构审阅，2026-06-21）

10 条 ADR + 6 条细则：
- 角色拆 C/A/S/SYSTEM（ADR-0001），操作归属 A/S/SYSTEM（ADR-0002）
- 转接补全生命周期 + 限深 3 + 失败回流（ADR-0003）
- 留言回复回灌下次会话（ADR-0004）
- 评价拆 skipped/expired 4 态 + 催评限 2 次（ADR-0005）
- 消息审核用户侧可见性（ADR-0006）
- 敏感词库改商城供给 + fail-open 不留红线（ADR-0007/0008）
- 挂起恢复归属回原客服、离线即重分（ADR-0009）
- 客户信息一期 3 有效 Tab + 行为灰显（ADR-0010）
- 组织归属三层模型确认（ADR-0011）

二轮边角（Q1-Q7，2026-06-22）：转接重排 2 次终止兜底、催评封顶 2 次+主动跳过提前停、转接失败回流消息退原客服、差评追踪=记录跟进+手动补偿不挂绩效、expired 阈值 48h、挂起优先于离线宽限。

---

> **版本**：后台 V0.5（原型同步修订） | **最后更新**：2026-06-26
> **本稿定位**：重写「一期后台」的产品需求，**替代 `PRD-OCS-V1.0.md` 中与后台相关的全部内容**。V1.0 后台部分的主要问题：① 功能与 admin 原型对不上（凭印象写，字段/状态/默认值多处偏差）；② 后台与用户端混在一篇、不够聚焦；③ 功能范围需重新界定。本稿全部修正。
> **范围口径**：**一期后台 11 个模块** = 侧边栏导航（数据仪表盘 / 客服工作台 / 待办事项 / 会话记录 / 会话转接 / 评价管理 / 知识文档 / 话术库 / **FAQ([新增])** / 系统设置）+ 客户信息页（不在导航，从工作台进入，本期作为独立模块纳入）。用户端、2/3 期（工单/AI/绩效看板/技能组等）一律 Out of Scope。
> **事实来源**：功能清单以 `prototype-cs/admin/` 原型 HTML 为准（逐页提取）。凡原型未实现、但 PRD 需要约束的点，统一用「[待补]」标注，不冒充原型既有。
...
> **模块简称**：本稿各模块以中文简称指代——仪表盘 / 工作台 / 待办 / 会话记录 / 客户信息 / 转接 / 评价 / 知识文档 / 话术 / FAQ / 设置（见 §1.1「简称」列）。
> **原则**：一页纸讲清楚，图大于文，只写差异不写常识，只写原型真实存在的功能。

## 版本说明

> **V0.5 修订**（原型同步，2026-06-26）：在 V0.4 基础上同步 6/25 后改动：① [修改] **路由规则->客服分配规则** 改名；**紧急度判定->紧急度设置** 改名；② [已解决] **B13 已解决**（较急档去掉，T=首次响应超时子卡片可配）；③ [已解决] **B14 已解决**（`modal-faq.html` 已建+FAQ 分类管理弹窗）；④ [增加] 自动回复区块新增 **FAQ 命中策略**（推送首条/全部/按优先级）；⑤ [修改] 排队超时从路由移至会话超时子卡片；⑥ [已解决] **并发编辑需求去掉**（不再要求编辑锁定）；⑦ [修改] 设置区块名更新：客服分配规则/会话超时设置/紧急度设置/敏感词/自动回复。
>
> **V0.4 修订**（原型同步，2026-06-26）：**核对当前 `admin/*.html` 原型，修正 8 处结构性偏差**：① [重组] **Nav 重组**：旧 3 组(工作台/服务管理/配置)-> **仪表盘(独立项) / 会话管理(组) / 知识库(组:话术·FAQ·知识文档) / 客服设置(独立项)**（`nav.js` 同步）；② [新增] **FAQ 独立页** `admin-faq.html`（6 分类/on-off/FAQ卡片推送），从知识库页拆出为独立模块；③ [降级] **快捷回复再次降级**：独立页 `.deprecated` -> 并入话术库为 `quickreply` 业务分类(场景=general)；话术业务分类 4->**5**；④ [标签] **知识库页标签改名**：nav label「知识库」->「知识文档」（实体名/文件名不变）；⑤ [修改] **设置 6->5 区块**：欢迎语+离线语并入「自动回复(autoreply)」section；⑥ [超时] **超时拆 6 生命周期子阶段**：排队等待/首次响应/轮次响应/用户静默/转接/挂起，每阶段独立阈值+动作；⑦ [增加] 评价补 `skipped` 态 + 催评/一键催评；⑧ [标记] 工作台会话卡新增 `即将超时(warn)`/`转接入(transferred)` 标签。详见各模块「V0.4」标注。
>
> **历史修订（V2.1/V2.2/V0.3）见文末 [附录·修订记录](#附录修订记录)。**
