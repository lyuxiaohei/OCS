...
> 生成日期：2026-06-12  
> 覆盖范围：全部 20 个原型页面（管理端 15 页 + 用户端 5 页）；另含 1 个 **2 期规划、暂无原型** 的工单系统流程（第 16 节）
...
**文件**：无原型页面（2 期规划，B8-工单系统）  
**分期**：2期-体验提升  
**功能**：跨部门流转处理用户问题 — 工单字段 / 状态流转 / 操作 / 与我相关 / 消息联动

```mermaid
flowchart TD
    ENTER([创建工单入口]) --> ENTRY{来源}
    ENTRY -->|会话页「创建工单」| FROM_CHAT[为当前会话客户创建]
    ENTRY -->|客户详情页「创建工单」| FROM_CUST[为该客户创建]
    ENTRY -->|用户消息自动生成| FROM_AUTO[系统自动生成工单]
    ENTRY -->|会话框一键生成| FROM_QUICK[客服一键创建]
    ...
    FORM[工单表单<br/>标题 / 分类（售后·投诉·技术故障）<br/>描述 / 附件 / 优先级 / 指派人] --> SUBMIT[提交工单]

    SUBMIT --> STATUS{工单状态流转}
    STATUS -->|未分配| UNASSIGNED[等待指派]
    UNASSIGNED -->|指派人接单| PROCESSING[处理中]
    STATUS -->|待处理| PENDING[已指派待处理]
    PENDING -->|接单| PROCESSING

    PROCESSING --> OPS{工单操作}
    OPS -->|转单| TRANSFER[转给其他客服/部门]
    OPS -->|回复| REPLY[添加处理回复]
    OPS -->|加抄送| CC[抄送相关人员]
    OPS -->|状态变更| CHANGE{变更状态}

    CHANGE -->|已解决| RESOLVED[已解决]
    CHANGE -->|处理中| PROCESSING
    RESOLVED -->|确认关闭| CLOSED([已关闭])

    ENTER --> MYVIEW{「与我相关」筛选}
    MYVIEW -->|待我处理| M1[我接单的工单]
    MYVIEW -->|我创建的| M2[我提交的工单]
    MYVIEW -->|抄送我的| M3[抄送给我的工单]
    MYVIEW -->|我处理的| M4[我处理过的]
    MYVIEW -->|我解决的| M5[我标记解决的]

    style ENTER fill:#909399,color:#fff
    style FORM fill:#e6a23c,color:#fff
    style CLOSED fill:#67c23a,color:#fff
    style FROM_AUTO fill:#722ed1,color:#fff
    style FROM_QUICK fill:#722ed1,color:#fff
```
...
> **文档结束**  
> 本文档覆盖全部 20 个原型页面 + 1 个 2 期规划（工单系统），共包含 21 个 Mermaid 流程图 + 2 个跨页面旅程图。
