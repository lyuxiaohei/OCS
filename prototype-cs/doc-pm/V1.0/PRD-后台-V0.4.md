# OCS（在线客服）— 后台 PRD V0.4

> **版本**：后台 V2.3（原型同步修订） | **最后更新**：2026-06-26
...
| B-12 | 系统设置 | 设置 | `admin-settings.html` | 🆕 **5 区块**：路由规则/会话超时(**6 生命周期子阶段**)/紧急度/敏感词/自动回复(欢迎语+离线语合并)。dirty 保存 | P1 |
...
> 🆕 **V0.4**：**5 区块**（路由规则/会话超时/紧急度/敏感词/自动回复）。欢迎语+离线语并入「自动回复」section；会话超时拆为 6 生命周期子阶段。
...
> 🆕 **V0.4 重组**：欢迎语+离线语并入「自动回复 #autoreply」；路由仅轮询/空闲度（无 VIP/技能组）；超时拆 6 生命周期子阶段。

| 区块 | 字段 | 控件 | 默认值 | 说明 |
|------|------|------|--------|------|
| 路由 | `is_auto_accept` | switch | 开（自动接入）| 新会话按分流策略自动分配 |
| 路由 | 分流策略 `routing_strategy` | radio 2 | 轮询分配（均衡）| 按空闲度（无 VIP/技能组，属 2-3 期）|
...
| 紧急度 | 紧急类目关键词组 `urgency_high_keywords` | 词表（可增删） | 退款/退货/投诉/差评/破损/物流异常 | 命中 → 紧急（钱损/投诉风险，不论等待时长）|
...
> **紧急度规则**见 [待办.2.4](#待办24-紧急度判定规则)：等待 ≥ 首次响应阈值 T 或命中紧急关键词 → 紧急；否则一般。⚠️ T 写死不可配（见 B13）。
