...
| 3 | title | 标题 | varchar(200) | 是 | 系统生成(三级类型 + 工单号) | 系统生成;不再人工填写(2026-07-24 三轮定) |
...
| 12 | user_id | 客户 ID | varchar(32) | 否 | FK → CUSTOMER | 会话 / 订单建单自动绑定;手动创建 1 期不强制选客户(可通过关联订单 / 联系方式追溯,2026-07-24 三轮定) |
...
- **入口**: 客服工单(独立一级菜单)
- **关键元素**: 状态分栏(未处理 / 处理中 / 已完工 + 全部)、统计摘要条(4 项)、筛选条(受理客服 / 工单类型三级联动 / 完成期限 / 优先级 / 状态 / 客户昵称手机号 / 处理人 / **SKU物流编码** / **来电号码** / 关联订单号 / **商品名称** / **项目名称** / 日期)、数据表格(行勾选)、导出(弹窗)
- **使用角色**: R-A / R-S
...
| UC-TL-02 | 筛选查询 | R-A | 受理客服 / **工单类型(三级联动)** / **完成期限** / 优先级 / 状态 / 客户昵称手机号 / 处理人 / **SKU物流编码** / **来电号码** / 关联订单号 / **商品名称** / **项目名称** / 日期 组合过滤 |
...
| 客户 | 昵称 + 手机号(**全号,不脱敏**;2026-07-24 四轮:客服作业场景需直接外呼/核对) | CUSTOMER 表;不显示头像 |
| 处理人 / 更新时间 | 处理人 + 更新时间(合并列,处理人 / 「未分配」) | — |
| 受理时间 | 工单创建时间(= `created_at`,BRD「受理时间」) | **表头点击排序**(升 / 降 / 默认三态;2026-07-24 四轮新增) |
