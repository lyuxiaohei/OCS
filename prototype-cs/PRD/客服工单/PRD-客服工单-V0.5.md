...
>
> **原型目录**: `prototype-cs/admin/` (`admin-ticket.html` / `admin-ticket-settings.html` / `admin-order-list.html` / `admin-workbench.html`)
>
...
> 36. **新增「接单」操作**:指派后客服手动确认接单→状态变处理中;保持 3 态状态机。
> 37. **补充通知**:建单/修改时通知处理人(BR-150);超时自动通知处理人+主管(BR-161/162);完工记录不可修改强化。
> 38. **订单受理入口 1 期可用**:`admin-order-list.html` 已有「受理」按钮,创建方式③从 2 期转入 1 期。
> 39. **工作台同步调整**:`admin-workbench.html` 需同步新增「接单」操作入口 + 建单/转工单通知 + 右栏工单区;由工作台负责人另排,属 1 期范围(见 §零 0.15)。
>
> **V0.5 分类/责任/期限决策**:
...
| 0.8 | **新建 API** | `/api/backend/ticket/*` | 工单 CRUD + 分派 + 接单 + 备注 + 催办 + 导出(见十二) |
...
| 0.15 | **改工作台** | `admin-workbench.html` | 操作栏 +「转工单」+「接单」按钮;右栏 +「该客户工单」区;**由工作台负责人另排,属 1 期范围** |
| 0.16 | **已就绪(1 期)** | `admin-order-list.html` | 订单列表「受理」→建单入口(创建方式③,已在原型实现) |
...
| 范围 | 内容 |
|------|------|
| **In Scope (1 期)** | 工单列表与查询 / 工单详情与处理 / **会话转工单** / 工单分派(手动 + 轮询) / **「接单」操作** / 工单回写会话通知 / **三级分类 + 责任承担方配置** / **受理字段族 + 订单自动带出** / **批量追加订单(1 对多)** / **催办** / **附件上传** / **完成期限+倒计时+超时通知** / 仪表盘扩展工单指标卡 / **订单列表页「受理」建单** / **工作台工单入口同步**(由工作台负责人另排) |
...
| 代号 | 角色 | 客服工单职责 |
|------|------|-------------|
| R-A | 客服 | 会话转工单、处理工单、接单、内部备注、转派、催办、回复用户(跳工作台)、完工、上传附件 |
...
| 2 | 工单详情 | 抽屉(嵌入工作台与列表) | ✅ 已建 | 含催办 / 附件 / 倒计时 / 接单 / 新联系字段 |
| 3 | 工单设置 | admin-ticket-settings.html | ✅ 已建 | 三级分类 + 责任承担方 + 优先级 + 完成期限值 增删改 |
| 4 | 订单列表 | admin-order-list.html | ✅ 已建 | 订单「受理」→建单入口(1 期可用) |
| 5 | 客服工作台 | admin-workbench.html | 🔧 改现有 | 操作栏 +「转工单」+「接单」;右栏 +「该客户工单」区;**由工作台负责人另排** |
...
> 工单列表为独立页(客服工单一级菜单);工单详情为抽屉;工单设置归属「客服设置」。订单管理页「受理」建单入口已就绪(admin-order-list.html)。
...
| 4 | category | 受理业务类型(一级) | varchar(10) | 是 | 咨询业务 / 投诉业务 | 创建时选(见三.3) |
| 5 | sub_category | 咨询/投诉类别(二级) | varchar(20) | 是 | 联动一级:产品咨询/物流咨询/产品投诉/物流投诉(BRD §4.4 字典) | 创建时选(见三.3) |
| 6 | type | 具体咨询/投诉类型(三级) | varchar(30) | 是 | 联动二级(BRD §4.4 字典,可后台增删改) | 创建时选(见三.3) |
| 7 | priority | 优先级(紧急程度) | varchar(10) | 否 | 一般 / 紧急 / 特急 | 创建时选(可配,见十.2;分类/排序用,不驱动 SLA) |
| 8 | status | 工单状态 | varchar(20) | 是 | 未处理 / 处理中 / 已完工,默认「未处理」 | 状态机流转 |
| 9 | responsible_party | 责任承担方 | varchar(30) | 投诉类必填 | 宏兆 / 供应商 / 东莞邮政 / 昆山吉波(可配) | 投诉类创建时必选(见三.3) |
| 10 | deadline | 完成期限 | varchar(10) | 否 | 1天 / 7天 / 15天(设置页可配) | 创建时选;驱动 deadline_at |
| 11 | deadline_at | 期限截止时间 | datetime | 否 | `created_at` + deadline 时长 | 系统自动计算;用于倒计时展示 + 超时扫描 |
| 12 | user_id | 客户 ID | varchar(32) | 是 | FK → CUSTOMER | 创建时绑定 |
| 13 | session_id | 来源会话 ID | varchar(32) | 否 | FK → SESSION | 会话转工单时写入(人工处理过的会话) |
| 14 | handler_admin_id | 处理人 | varchar(32) | 否 | FK → ADMIN | 指派时写入 |
| 15 | source | 来源 | tinyint | 是 | 0=用户 / 1=客服 / 2=系统 / 3=订单受理 | 创建时确定(会话转工单 / 手动创建=客服;从订单受理创建=订单受理) |
| 16 | created_at | 受理时间 | datetime | 是 | 系统时间,精确到秒 | 系统自动;不可修改(BRD §4.2) |
| 17 | description | 工单描述(受理内容) | text | 否 | — | 创建时填写;详情可编辑,保留历史版本 |

> 相比 V0.4:① `category`+`type`(两级)→ **`category`+`sub_category`+`type`(三级)**,一级用词对齐 BRD「咨询业务/投诉业务」;② 新增 **`deadline`/`deadline_at`**(完成期限代替 SLA);③ 移除 `handler_group`(处理组,BRD 无对应字段)、`score`(评分,本期不实现,BRD 无)、`finished_at`(完工时间走 event 流水记录)。**无 SLA 字段**(沿用 V0.3)。
>
> **BRD §4.2 字段落位**:受理编号 / 受理时间(不可改)= 序号 2 / 16;受理人 = 序号 14(handler);紧急程度 = 序号 7;受理业务类型 / 类别 / 类型 = 序号 4 / 5 / 6;责任承担方 = 序号 9;完成期限 = 序号 10;受理内容 = 序号 17。其余联系方式 / 订单 / 业务字段落 `extension`(三.2.6)。

#### 三.2.2 ~ 三.2.5(事件流水 / 备注 / MESSAGE 扩展 / SESSION 扩展)

沿用 V0.4,新增事件类型:
- `customer_chat_ticket_events`:`ticket.created` / `ticket.assigned` / `ticket.accepted`(V0.5 新增) / `ticket.replied` / `ticket.transferred` / `ticket.reminded`(催办) / `ticket.overdue`(V0.5 新增,超时标记) / `ticket.finished`。`operator_type` = admin / customer / system。**所有 event 记录不可修改、不可删除。**
...
- **入口**: 客服工单(独立一级菜单)
- **关键元素**: 状态分栏(待处理 / 处理中 / 已完工 + 全部)、统计摘要条(4 项)、筛选条(受理客服 / 工单类型三级联动 / 完成期限 / 优先级 / 状态 / 客户昵称手机号 / 处理人 / 来源会话 / 关联订单号 / **商品名称** / **项目名称** / 日期)、数据表格、导出
- **使用角色**: R-A / R-S
...
| UC-TL-02 | 筛选查询 | R-A | 受理客服 / **工单类型(三级联动)** / **完成期限** / 优先级 / 状态 / 客户昵称手机号 / 处理人 / 来源会话 / 关联订单号 / **商品名称** / **项目名称** / 日期 组合过滤 |
...
| 标题 | 工单标题 | — |
| 工单类型 | 一级 + 二级 + 三级(如「投诉业务 · 物流投诉 · 物流破损退货」) | tag |
| 优先级 | 一般 / 紧急 / 特急 | tag,紧急红色 |
| 责任承担方 | 宏兆 / 供应商 / …(投诉类) | 投诉类显示 |
| 完成期限 | 1天 / 7天 / 15天 / — | tag |
| 剩余时间 | "3天·2小时" / "已超时·1天"(红色) | 倒计时;超时行红色高亮 |
...
| 操作 | 详情 / 回复(跳工作台) / **接单**(未处理+已分配) / 转派(转单) / 催办 / 指派(未处理+未分配) | 按状态 + 角色显示;**完工在详情抽屉底部(非行操作,对齐六.2)** |

> 相比 V0.4:类型列→三级;补**完成期限列+剩余时间(倒计时)列**;行操作补**接单**(有 handler 未接单时显示);移除 handler_group/score/finished_at 列(字段已删)。
...
- **入口**: 客服工作台 > 会话操作栏「转工单」/ 工单列表「新建工单」/ 订单管理页「受理」
...
| ③ 从订单受理创建 | 订单管理页「受理」 | 基于订单建单,自动带出商品 / 收货 / 项目名;admin-order-list.html 入口已就绪 |
...
工单抽屉:基本信息 + 状态流转 + 内部备注 + 操作日志 + 来源会话 + 附件 + 倒计时。工单描述可编辑(保留历史版本);回复用户跳转会话工作台。
...
| 未处理 | 处理中 | 接单 | handler 匹配当前用户 | R-A | ✅ 1 期(V0.5 新增) |
| 处理中 | 处理中 | 转派 | 换 handler | R-A | ✅ 1 期 |
| 处理中 | 已完工 | 客服完工 | — | R-A | ✅ 1 期 |

> 3 态(未处理 / 处理中 / 已完工)。指派仅在未处理态指定 handler,不改变状态;接单=客服确认领取→状态变处理中。已完工为终态,不可操作,继续处理需新建工单。

### 六.2 功能用例与验收(UC-TD / AC-TD)

沿用 V0.4 并修订:查看详情 / 指派转派 / 接单 / 内部备注 / 状态变更 / 查看来源会话 / 完工。**V0.5 新增**:接单;完成期限倒计时。已完工为终态,不可操作。
...
| **UC-TD-14** | **接单** | R-A | 未处理+已分配 → 客服点击「接单」确认领取 → 状态变处理中(写 `ticket.accepted`) |

> 相比 V0.4:新增 UC-TD-14 接单;催办扩展为人工+系统双触发(超时自动)。状态操作按钮集中抽屉底部。

---

## 七、工单分派(代号:AS)

1 期:手动指派 + 轮询自动分派。**指派不改变状态(留在未处理),接单后才变处理中。** 2 期:规则型分派(按类型 / 项目 / 客户等级 / 负载 / 技能)。

- **使用角色**: R-A(指派 / 接单 / 转派)、R-SYS(自动分派、超时扫描)
- **cron 调度**: 轮询自动分派每 **1 分钟**扫描未处理无 handler 的工单;**超时扫描每 30 分钟**扫描 deadline_at < now 且未完工工单

### 七.1 功能用例与验收(UC-AS / AC-AS)

沿用 V0.4:手动指派 / 轮询自动分派(跳过满载,全满载进待分配池通知主管)/ 转派(带审计)。V0.5 新增:接单(指派后客服手动确认)、超时自动通知。
...
| 接单 | 客服确认领取 | **status=处理中** | 写 `ticket.accepted`(V0.5 新增) |
...
> 对齐 3 态状态机(六.1):创建→未处理;指派→未处理(有 handler);接单→处理中;已完工=终态。**所有 event 记录不可修改、不可删除。** 无「待回复 / 阻塞 / 重启 / 关闭」阶段。
...
| # | 操作 | 方法 | 路径 | 关键参数 | 异常码 |
|---|------|------|------|----------|--------|
| 1 | 工单列表 | GET | `/api/backend/ticket/list` | 筛选(category/sub_category/type/deadline/priority/status/handler/商品名/项目名/date) | 空数据 |
| 2 | 工单详情 | GET | `/api/backend/ticket/detail` | ticket_id | 无权限 / 不存在 |
| 3 | 创建工单(含转工单) | POST | `/api/backend/ticket/create` | category / sub_category / type / deadline / priority / title / responsible_party(投诉类) / user_id / order_ids[] / session_id / extension | 参数缺失 / order_id 无效 / 投诉类缺责任承担方 / 提交部门或反馈方式未填 |
| 4 | 状态变更 | POST | `/api/backend/ticket/update` | ticket_id / action(**assign / accept / transfer / finish**) | 非法状态迁移(3 态) |
| 5 | 接单 | POST | `/api/backend/ticket/accept` | ticket_id | 非当前处理人 / 状态非未处理 |
| 6 | 指派 / 转派 | POST | `/api/backend/ticket/assign` | ticket_id / handler_admin_id | 目标满载 |
...
> V0.5 新增:**#5 接单**;#3 创建参数增加 `sub_category`/`deadline`;actions 增加 `accept`;#8 配置扩为三级分类+责任承担方+优先级+完成期限值。
...
|--------|--------------|-----------------|----------|
| 会话转工单 | 会话状态机 + 工作台操作栏 | 操作栏增「转工单」+「接单」;转后会话 end | **改现有** |
| 客户关联 | CUSTOMER 表 + 客户 360 画像 | TICKET.user_id FK | **复用** |
| 订单关联 | 客户信息-交易 Tab(OMS) | TICKET_ORDER(1 对多) | **复用** |
| 通知通道 | csNotify + 企业微信 | 状态回写 / 转派 / 催办 / 超时 / 建单通知 复用通道 | **复用** |
...
| 2 | 工单 CRUD API + 状态机(3 态,含 accept 接单)+ 三级分类 + 责任承担方 + 完成期限校验 | 1 |
| 3 | 会话转工单(工作台操作栏 + 上下文预填 + 会话自动 end) | 2 + 现有工作台 |
| 4 | 手动指派 + 轮询自动分派(指派不改变状态) | 2 |
| 4.5 | **「接单」操作**(指派后客服确认领取→状态变处理中) | 2 |
...
| 8 | **催办(#13)+ 附件(#14)** | 2 |
| 9 | 工单列表页(状态分栏 + 三级分类筛选 + 完成期限筛选 + 倒计时列)+ 详情抽屉(含接单+新联系字段) | 2 |
| 10 | 工单设置页(三级分类 + 责任承担方 + 优先级 + **完成期限值**) | 现有 settings 模式 |
| 11 | 仪表盘扩展工单指标卡(含超期工单数) | 2 + 现有 dashboard |
| 12 | **工作台同步:操作栏+「接单」,右栏+「该客户工单」区,建单通知展示** | 由工作台负责人另排(属 1 期范围) |
...
| ⚠️ 8 | 工作台右栏「该客户工单」区 + 操作栏「接单」 | 需在工作台新增工单功能入口 | **由工作台负责人另排,属本次 1 期范围**;需与工作台 PRD 对齐 |
| ⚠️ 9 | 指派与接单分离后的流程 | 指派不改变状态→接单才变处理中,若客服不接单? | 主管督促+催办;未接单工单在「未处理」栏展示 |
...
| 原型文件 | 待同步改动 | 对应 PRD |
|---------|-----------|---------|
| `modal-create-ticket.html`(建单弹窗) | 分类选择器三级级联(受理业务类型→咨询/投诉类别→具体类型);投诉类显「责任承担方」;联系方式(含新XXX字段)/提交部门(必填)/反馈方式(必填)/完成期限(下拉)字段;order-picker 批量多选(1 对多) | 三.3 / 三.2.6 / 五.2 / 五.3 |
| `admin-ticket.html`(列表) | 状态分栏;类型列显示三级;补责任承担方列(投诉类);补完成期限列+倒计时列(超时红色);筛选补三级联动+完成期限+商品名+项目名;行操作补接单+催办 | 四.1 / 四.2 |
| `drawer-ticket-detail.html`(详情) | 类型展示三级;补新联系人字段;补倒计时展示;补「接单」按钮(未处理+已分配态);补「催办」按钮(UC-TD-12);补附件/追加材料区(UC-TD-13) | 六.2 |
| `admin-ticket-settings.html`(设置) | 配置改三级分类(一级枚举+二级可配+三级可配)+责任承担方+优先级+完成期限值(含数量) | 十.2 |
| `admin-workbench.html`(工作台) | 操作栏补「接单」入口;右栏补「该客户工单」区;新增建单/超时通知展示;**由工作台负责人另排** | 见 §零 0.15 |

---

*本稿 V0.5 在 V0.4 基础上补齐 BRD V1.5 剩余差异:字段对齐(新XXX恢复/双座机/提交部门必填/反馈方式必填)、分类三级化、完成期限代替SLA(倒计时+超时通知)、新增接单操作、补充建单/修改/超时通知。消费者自助/自动/AI建单、规则分配、OMS深度集成归 2/3 期。V0.4 已归档于 `PRD/客服工单/archive/`。标注 🔴 项需向技术/业务确认(OMS 字段可用性);工作台同步由工作台负责人另排。*
