...
| 0.12 | **新建页面** | `admin-ticket-detail.html` | 工单详情独立页(列表行「详情」进入;含催办 / 附件 / 倒计时 / 接单) |
...
| 2 | 工单详情 | admin-ticket-detail.html | ✅ 已建 | 独立页(列表行「详情」进入),含催办 / 附件 / 倒计时 / 接单 / 新联系字段 |
...
| 6 | 客服工作台 | admin-workbench.html | 🔧 改现有 | 操作栏 +「转工单」+「接单」;右栏 +「该客户工单」区;**由工作台负责人另排** |
...
| 操作 | 详情 / 回复(跳工作台) / **接单**(未处理+已分配) / 转派(转单) / 催办 / 指派(未处理+未分配) | 按状态 + 角色显示;**完工在详情页底部(非行操作,对齐六.2)** |
...
- **入口**: 客服工作台 > 会话操作栏「转工单」/ 工单列表「新建工单」/ 订单管理页「创建工单」
...
> 相比 V0.4:新增 UC-TD-14 接单;催办扩展为人工+系统双触发(超时自动)。状态操作按钮集中详情页底部。
...
| 8 | **催办(#13)+ 附件(#14)** | 2 |
| 9 | 工单列表页(状态分栏 + 三级分类筛选 + 完成期限筛选 + 倒计时列)+ 详情页(含接单+新联系字段)+ 建单页(三级级联 + 批量追加) | 2 |
...
| `admin-ticket-detail.html`(详情独立页) | 类型展示三级;补新联系人字段;补倒计时展示;补「接单」按钮(未处理+已分配态);补「催办」按钮(UC-TD-12);补附件/追加材料区(UC-TD-13) | 六.2 |
...
> 注(2026-07-22 已同步):建单与详情以**独立页**形态落地(`admin-ticket-create.html` / `admin-ticket-detail.html`),替代原「弹窗 / 抽屉」设计;列表 / 设置 / 订单建单入口 / 工作台「转工单」按钮均已同步,工作台其余改造(接单入口 / 右栏工单区)仍由工作台负责人另排。
