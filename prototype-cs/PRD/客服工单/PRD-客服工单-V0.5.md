...
- **入口**: 客服工单(独立一级菜单)
- **关键元素**: 状态分栏(未处理 / 处理中 / 已完工 + 全部)、统计摘要条(4 项)、筛选条(受理客服 / 工单类型三级联动 / 完成期限 / 优先级 / 状态 / 客户昵称手机号 / 处理人 / 来源类型 / 关联订单号 / **商品名称** / **项目名称** / 日期)、数据表格、导出
- **使用角色**: R-A / R-S
...
| UC-TL-02 | 筛选查询 | R-A | 受理客服 / **工单类型(三级联动)** / **完成期限** / 优先级 / 状态 / 客户昵称手机号 / 处理人 / 来源类型 / 关联订单号 / **商品名称** / **项目名称** / 日期 组合过滤 |
...
| UC-TL-06 | 编辑工单信息 | R-A | 行「编辑」→ 抽屉改单(标题 / 三级分类 / 责任承担方 / 优先级 / 完成期限 / 联系方式 / 业务字段);**已完工终态不可改**;改完成期限按修改时刻 + 新期限重算截止;改为投诉类必校验责任承担方;写操作日志(字段级旧值→新值);详情页元信息卡同一入口;编辑 / 创建 / 详情三处排版一致(分区 + 标签左置),降低学习成本 |
...
| 标题 | 工单标题 | — |
| 工单类型 | 一级 / 二级 / 三级 各一行(3 行展示,如「投诉业务 / 物流投诉 / 物流破损退货」) | 列宽加大 |
...
| **15** | **编辑工单信息** | POST | `/api/backend/ticket/edit` | ticket_id / title / category / sub_category / type / responsible_party / priority / deadline(按修改时刻+新期限重算 deadline_at) / extension(联系方式+业务字段) | 已完工终态不可改 / 投诉类缺责任承担方 |
