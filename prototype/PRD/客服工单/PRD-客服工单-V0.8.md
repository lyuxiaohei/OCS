...
> **版本**: V0.8 | **日期**: 2026-09-01 | **状态**: 评审稿(开发评审后修订)
>
...
>
> **V0.8 变更(2026-09-01)**: **移除「催办」与「回复用户」功能**(列表与详情入口、`ticket.reminded` 事件、`POST /ticket/remind` API、人工催办链路全删;超时扫描/超时标记/超时通知保留);列表行操作拆平(操作≤3 项不再收 ⋯ 下拉),未处理/已完工行仅留详情,与原型同步。
>
> **核心口径**:
> - 三级分类:`category`(一级 受理业务类型) + `sub_category`(二级 咨询/投诉类别) + `type`(三级 具体类型),字典后台可配,初始字典取自 BRD §4.4。
> - 完成期限代替 SLA:`deadline`(1天/7天/15天,设置页可配)+ `deadline_at` 自动计算;列表/详情倒计时,超时标记,cron 每 30 分钟扫描并通知处理人+主管(V0.8:人工催办移除,超时链路保留)。
...
| 0.8 | **新建 API** | `/api/backend/ticket/*` | 工单 CRUD + 分派 + 受理 + 备注 + 导出(见十二;V0.8:催办 API 移除) |
...
| 0.12 | **新建页面** | `admin-ticket-detail.html` | 工单详情独立页(列表行「详情」进入;含附件 / 倒计时 / 受理) |
...
| 范围 | 内容 |
|------|------|
| **In Scope (1 期)** | 工单列表与查询 / 工单详情与处理 / **会话转工单** / 工单分派(手动 + 轮询) / **「受理」操作** / 工单回写会话通知 / **三级分类 + 责任承担方配置** / **受理字段族 + 订单自动带出** / **批量追加订单(1 对多)** / **附件上传** / **完成期限+倒计时+超时通知** / 仪表盘扩展工单指标卡 / **订单列表页「创建工单」建单** / **工作台工单入口同步**(由工作台负责人另排) |
| **Out Scope(本期不做)** | **催办与回复用户**(V0.8 移除,详见状态机与 API 清单);**消费者侧自助提交工单**(2 期);**触发式自动建单 / AI 智能建单**(2 / 3 期);**规则型分配**(按类型 / 项目 / 客户等级 / 技能,2 期);轮次响应 / 关单确认 / 母子工单 / 审批 / 技能组分派 / 流转触发器 / 完整报表 / OMS 深度集成(退款退货换货直连);**工单评价**(2 期) |

> **定位**:客服工单是 OCS 客服后台 **V0.9 的迭代增量**,在已有会话 / 客户 / 通知 / 角色基础设施上,增加「工单」事务实体 + 会话协同接口。会话解决即时问题,工单承接需跨部门 / 需订单系统操作的售后问题(退款 / 退货 / 换货 / 投诉 / 补发 / 价格干预)。
>
> **本期工单为内部处理,用户不可见**(消费者自助建单 2 期)。以完成期限(1天/7天/15天,设置页可配)代替 SLA,到期倒计时提醒 + 超时自动标记 + 通知处理人与主管(V0.8:人工催办移除)。

### 一.1 角色列表

| 代号 | 角色 | 客服工单职责 |
|------|------|-------------|
| R-A | 客服 | 会话转工单、处理工单、工单跟进(首次跟进自动受理)、发起售后/补偿、完工、上传附件;对客回复自行回会话工作台(V0.8:工单侧无入口) |
| R-S | 客服主管 | 查看全部工单、工单分类 / 责任承担方 / 完成期限值配置、催促跟进、导出;2 期含审批 |
...
| 2 | 工单详情 | admin-ticket-detail.html | ✅ 已建 | 独立页(列表行「详情」进入),含附件 / 倒计时 / 受理 / 新联系字段 |
...
- `customer_chat_ticket_events`:事件类型 `ticket.created` / `ticket.assigned` / `ticket.accepted` / `ticket.edited`(编辑工单信息:字段级旧值→新值,UC-TL-06) / `ticket.replied` / `ticket.overdue`(超时标记) / `ticket.finished`。`operator_type` = admin / customer / system。**所有 event 记录不可修改、不可删除。**(O5,2026-09-01 评审:**操作日志 UI 1 期不开发**,本表仅作底层流水保留,后续接入统一操作日志系统;V0.8:`ticket.reminded` 催办事件随催办功能移除)
...
| 操作 | 未处理:详情/编辑;处理中:详情/编辑/发起售后/发起补偿(**平铺行内,操作 ≤3 项不收下拉**,V0.8);已完工:详情 | 按状态显示;**完工 在详情页底部操作栏(非行操作,对齐六.2)**;催办/回复用户已移除(V0.8) |
...
工单详情页:基本信息 + 状态流转 + 工单跟进 + 来源会话 + 附件 + 倒计时(**操作日志 UI 1 期不开发**,O5,待统一操作日志系统;底层事件流水保留)。工单描述可编辑(**不保留历史版本**,O5)。对客回复由客服自行回会话工作台,工单侧无「回复用户」入口(V0.8 移除);来源会话字段保留「查看会话 →」带参跳转。
...
| UC-TD-10 | ~~回复用户~~ | — | **移除**(V0.8,2026-09-01:对客回复由客服自行回会话工作台,工单侧无入口;来源会话「查看会话 →」带参跳转保留) |
| UC-TD-11 | 添加工单跟进 | R-A | 工单内添加工单跟进(不回写会话;纯内部工单,**不做客户可见性区分与标注**——十八轮 39:14);**录入结果清晰可见**(纪要点 6) |
| UC-TD-12 | ~~催办~~ | — | **移除**(V0.8,2026-09-01:人工催办与超时自动催办均不做;超时扫描/标记/通知链路保留,见 UC-RB-03) |
...
| 底部操作栏 | 未处理:返回列表(跟进入口=工单跟进卡,首次跟进自动受理);处理中:发起售后 / 发起补偿 / 完工(全显,不下拉;V0.8:催办/回复用户移除);已完工:返回列表 |
...
| UC-RB-02 | (本期不做) | — | 对客回复由客服自行回会话工作台(V0.8:工单侧「回复用户」入口移除),工单内不做对客消息回写 |
...
| 超时 | cron 扫描 deadline_at < now且未完工 | 超时标记 | 写 `ticket.overdue` + 通知处理人 + 通知主管(V0.8:人工催办移除,仅保留此超时链路) |
...
| 13 | _(V0.8 移除)_ 催办 | — | 催办功能移除(2026-09-01),无独立接口 | — | — |
...
| 通知通道 | csNotify + 企业微信 | 状态回写 / 超时 / 建单通知 复用通道(V0.8:催办通知移除) | **复用** |
...
| ⚠️ 9 | 受理客服未及时跟进 | 建单指定受理客服后,若一直不首次跟进,工单停在未处理 | 主管督促;未处理栏展示(十七轮简化口径;V0.8:催办移除) |
...
| 原型文件 | 职责 | 对应 PRD |
|---------|------|---------|
| `admin-ticket.html`(列表) | 商城运营后台骨架;状态分栏(**tab 不带数量**,十八轮)+ 统计 4 卡片;筛选区收起/展开(默认收起,操作组占一格);三级联动筛选 + 完成期限 + 企业名称/**商城名称(输入框,十八轮)**;真分页 10 条/页;行勾选蓝底 + 全选=当前页;导出弹窗(仅主管);行操作按状态(V0.8:处理中=详情/编辑/发起售后/发起补偿 **平铺行内,不收 ⋯ 下拉**;未处理/已完工仅详情/编辑);**关联订单拆主/子订单两列,多单「+X」点击弹小弹窗列全部子订单号,子单号点击跳订单列表按子单号搜索(十八轮)**;**联系人筛选取值合并匹配收货+新联系人两套字段(A 方案,十八轮)**;mock 17 行示例数据(含临期倒计时/昆山吉波责任方/短信·客户邮件反馈/未处理两分流等场景) | 四 |
| `admin-ticket-detail.html`(详情) | 独立页;页头状态栏合并;主栏 2 列网格基本信息(含企业名称/商城名称/受理客服/受理时间)+ 关联订单(多单拆行,子订单号可点击跳订单列表,无单独查看详情按钮)+ 描述(历史版本)+ 附件;右栏 360px 工单跟进(延伸满高;首次跟进自动受理:未处理→处理中);操作日志移至主栏底部附件之后(随主栏滚动,降序,时间+内容+账号,无角色标签);底部操作栏:未处理仅返回、处理中=发起售后/发起补偿/完工(V0.8:催办/回复用户移除);已完工终态只读 | 六 |
